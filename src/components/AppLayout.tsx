...
import { Bell, Search, Building2, ChevronDown, User, Settings, LogOut, Hexagon } from 'lucide-react';
    ...
    <SidebarProvider>
      <div className="min-h-screen flex w-full bg-background">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-12 flex items-center justify-between border-b bg-card/80 backdrop-blur-md px-4 shrink-0 sticky top-0 z-10">
            <div className="flex items-center gap-2">
              ...
              {searchOpen ? (
                <div className="relative">
                  ...
                  <Input placeholder="Buscar..." className="h-8 w-[220px] md:w-[280px] pl-8 text-xs bg-muted/50 border-0 focus-visible:ring-1" autoFocus onBlur={() => setSearchOpen(false)} />
                </div>
              ) : (
                <button className="hidden sm:flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground px-2.5 py-1.5 rounded-md bg-muted/30 hover:bg-muted/50 transition-colors" onClick={() => setSearchOpen(true)}>
                  <Search className="h-3.5 w-3.5" /><span>Buscar...</span><kbd className="hidden lg:inline text-[10px] bg-background px-1.5 py-0.5 rounded border ml-4 font-mono">⌘K</kbd>
                </button>
              )}
            </div>
            <div className="flex items-center gap-1.5">
              <div className="hidden lg:flex items-center gap-1.5 text-[11px] text-muted-foreground border rounded-md px-2.5 py-1 bg-muted/20 font-medium">
          ...
          <main className="flex-1 overflow-auto p-4 md:p-6 scrollbar-thin">{children}</main>
        </div>
      </div>
    </SidebarProvider>
  );
